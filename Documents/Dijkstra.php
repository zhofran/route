	...
	*/
	function jalurTerpendek($arg_graph, $simpulAwal, $simpulTujuan){
		...
		if(!array_key_exists($simpulAwal, $arg_graph) || !array_key_exists($simpulTujuan, $arg_graph)){
			return json_encode(['status'=>'error','error'=>'simpul_input_tidak_ditemukan','teks'=>"could not find the input : $simpulAwal or $simpulTujuan", 'content'=>'']);
		}
			...
			*/
			for($perulanganSimpul = 0; $perulanganSimpul < count($simpulYangDikerjakan); $perulanganSimpul++)
			{
				...
				*/
				...
				{
					// ======================================================
					// # JIKA INDEX GRAPH ANDA SALAH, MAKA DIE() SAMPAI IF INI
					// misal :
					// graph [0][0]
					// graph [0][5] <-- indexnya 5 salah
					// ======================================================
					if(!isset($graph[ $simpulYangDikerjakan[$perulanganSimpul] ][$start_baris])){
						return json_encode(['status'=>'error', 'error'=>'index_graph_anda_salah', 'teks'=>'graph['.$simpulYangDikerjakan[$perulanganSimpul].']['.$start_baris.'] not found', 'content'=>'']);
					}
